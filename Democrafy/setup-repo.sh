...
# setup-repo.sh
# Script to scaffold the Democrafy project locally with full iOS client & Node backend
...
echo "=== Democrafy Local Repo Setup ==="

# 1) Create project directories
echo "Creating directories..."
mkdir -p Democrafy-iOS/{Models,Services,ViewModels,Views,Resources}
mkdir -p Democrafy-Backend

echo "Directories created."

# 2) Write iOS client files
# AppEntry.swift
cat > Democrafy-iOS/AppEntry.swift << 'EOF'
import SwiftUI
...
echo "Written: Democrafy-iOS/AppEntry.swift"

# Models
cat > Democrafy-iOS/Models/PartyRoom.swift << 'EOF'
import Foundation
...
cat > Democrafy-iOS/Models/SuggestedTrack.swift << 'EOF'
import Foundation
...
cat > Democrafy-iOS/Models/SpotifyDevice.swift << 'EOF'
import Foundation
...
cat > Democrafy-iOS/Models/Track.swift << 'EOF'
import Foundation
...
cat > Democrafy-iOS/Models/PlaybackState.swift << 'EOF'
import Foundation
...
# Services
cat > Democrafy-iOS/Services/SpotifyAuthService.swift << 'EOF'
import Foundation
import Combine
import CryptoKit
...
class SpotifyAuthService: NSObject, ObservableObject {
    ...
    let clientID = "YOUR_SPOTIFY_CLIENT_ID"; let redirectURI = URL(string: "democrafy://callback")!
...
cat > Democrafy-iOS/Services/SpotifyAPIService.swift << 'EOF'
import Foundation
import Combine
...
cat > Democrafy-iOS/Services/PartyBackendService.swift << 'EOF'
import Foundation
import Combine
...
# ViewModels
cat > Democrafy-iOS/ViewModels/LobbyViewModel.swift << 'EOF'
import Foundation
import Combine
...
cat > Democrafy-iOS/ViewModels/SearchViewModel.swift << 'EOF'
import Foundation
import Combine
...
cat > Democrafy-iOS/ViewModels/QueueViewModel.swift << 'EOF'
import Foundation
import Combine
...
cat > Democrafy-iOS/ViewModels/PlayerViewModel.swift << 'EOF'
import Foundation
import Combine
...
# Views
cat > Democrafy-iOS/Views/LobbyView.swift << 'EOF'
import SwiftUI
...
cat > Democrafy-iOS/Views/SearchView.swift << 'EOF'
import SwiftUI
...
cat > Democrafy-iOS/Views/QueueView.swift << 'EOF'
import SwiftUI
...
cat > Democrafy-iOS/Views/PlayerView.swift << 'EOF'
import SwiftUI
...
cat > Democrafy-iOS/Views/MockupViews.swift << 'EOF'
import SwiftUI
...
cat > Democrafy-Backend/package.json << 'EOF'
{
  "name": "democrafy-backend",
  "version": "1.0.0",
  "main": "server.js",
...
echo "Written: Democrafy-Backend/package.json"

cat > Democrafy-Backend/server.js << 'EOF'
...
echo "Written: Democrafy-Backend/server.js"
